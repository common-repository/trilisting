		...
		protected function render_fallback() {
			...
			<li id="accordion-panel-<?php echo esc_attr( $this->id ); ?>" class="<?php echo esc_attr( $classes ); ?>">
				<h3 class="accordion-section-title" tabindex="0">
					...
					?>
					<span class="screen-reader-text"><?php esc_html_e( 'Press return or enter to open this panel', 'trilisting' ); ?></span>
				</h3>
		...
		protected function render_content() {
			...
			<li class="panel-meta accordion-section redux-panel redux-panel-meta control-section<?php if ( empty( $this->description ) ) {
				...
				<span class="preview-notice"><?php
						...
						echo sprintf( esc_html__( 'You are customizing %s', 'trilisting' ), '<strong class="panel-title">' . esc_html( $this->title ) . '</strong>' );
					?></span>
				</div>
		...
		protected function content_template() {
			...
			<li class="panel-meta customize-info redux-panel accordion-section <# if ( ! data.description ) { #> cannot-expand<# } #>">
				<button class="customize-panel-back" tabindex="-1">
					<span class="screen-reader-text"><?php esc_attr_e( 'Back', 'trilisting' ); ?></span></button>
				<div class="accordion-section-title">
				<span class="preview-notice"><?php
						/* translators: %s is the site/panel title in the Customizer */
						echo sprintf( esc_html__( 'You are customizing %s', 'trilisting' ), '<strong class="panel-title">{{ data.title }}</strong>' );
					?></span>
					<# if ( data.description ) { #>
						<button class="customize-help-toggle dashicons dashicons-editor-help" tabindex="0" aria-expanded="false">
							<span class="screen-reader-text"><?php esc_attr_e( 'Help', 'trilisting' ); ?></span></button>
						<# } #>
				</div>
				<# if ( data.description ) { #>
